...
<div class="container-fluid">
    <h4 class="text-center text-primary p-2"><i
                class="fa fa-folder-open-o"></i> {$parametros['nomenclatura']} {$processo} -
    ...
    <div class="row">
        <div class="col-3">
            <div id="listaDocumentos" class="list-group overflow-auto" style="font-size: 12px;">
                    <a href="{$processo->getAnexoUrl()}{$processo->getCapa()}"
                       class="list-group-item list-group-item-action  {if !$processo->getApensado()} active{/if}  arquivo-processo">
                        <i class="fa fa-file"></i> <strong> {if $processo->getApensado()} Apenso {$p} <br>{/if} Capa do Processo</strong>
                    </a>
                    {foreach $processo->getComponentes(true, true) as $componente}
                            ...
                            {if $componente->getTramite()->gerarFormularioEletronico() eq true}
                                <a href="{$processo->getAnexoUrl()}{$componente->getTramite()->getNomeArquivoFormularioEletronico()}"
                                   class="list-group-item list-group-item-action arquivo-processo">
                                    ...
                                    <br/>
                                    <small>
                                        <i class="fa fa-clock-o"></i> {$componente->getTramite()->getDataEnvio()->format('d/m/Y - H:i')}
                        ...
                        {else if $componente->getAnexo()}
                            {if !is_int($componente->getAnexo()->getTamanho())}
                                <div class="d-flex">
                                <input type="checkbox" class="m-1" name="{$componente->getAnexo()->getId()}">
                                <a href="{$componente->getAnexo()->getArquivoCarimbado(true)}?v={time()}"
                                   class="list-group-item list-group-item-action arquivo-processo">
                                    ...
                                    <small>
                                        ...
                                        {" | "}
                                        <i class="fa fa-clock-o"></i> {$componente->getAnexo()->getDataCadastro()->format('d/m/Y - H:i')}
                                        {" | "}
                    ...
                    {foreach $processo->getApensos() as $apenso}
                        ...
                        <a href="{$apenso->getAnexoUrl()}{$apenso->getCapa()}"
                           class="list-group-item list-group-item-action  {if !$apenso->getApensado()} active{/if}  arquivo-processo">
                            <i class="fa fa-file"></i> <strong> {if $apenso->getApensado()} Apenso {$p} <br>{/if} Capa do Processo</strong>
                        </a>
            ...
            <button type="button" target="_blank"
               href="{$app_url}processo/download/id/{$processo->getId()}"
               class="btn btn-warning btn-block btn-download-processo" processo-numero="{$processo->getNumero()}" processo-exercicio="{$processo->getExercicio()}">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="btn-group col-md-12" role="group" aria-label="Basic example">
                            <button 
                                type="button" 
                                class="btn col-md-6  btn-outline-info btn-arquivo-anterior">
                                <i class="fa fa-arrow-left"></i> Anterior
                            </button>
                            <button 
                                type="button" 
                                class="btn col-md-6  btn-outline-info btn-proximo-arquivo">
                                Próximo <i class="fa fa-arrow-right"></i>
                            </button>
                        </div>
                    </div>
                    <iframe id="object_pdf"
                                base_url="{$app_url}lib/pdfjs/web/viewer.html?file="
