...
class FriendlyErrors {
    ...
    public static function checkExecutablePresence($executable) {
        if (file_exists($executable))
            return;

        //$cmd = stripos(PHP_OS, 'win') === 0 ? 'where.exe ' . Command::escape($executable) . ' > NUL 2>&1' : 'type ' . Command::escape($executable) . ' > /dev/null 2>&1';
        $cmd = stripos(PHP_OS, 'win') === 0 ? 'where.exe ' . Command::escape($executable) . ' > NUL 2>&1' : 'type ' . Command::escape($executable) . ' > /dev/null 2>&1';
        system($cmd, $exitCode);
    ...
    public static function checkCommandExecutionConvertText($exec, Command $command, $stdout) {
    ...
    public static function checkCommandExecution($command, $stdout) {
        $file = "{$command->getOutputFile()}.pdf";
