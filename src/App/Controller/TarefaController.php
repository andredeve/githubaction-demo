    ...
    function ordenar() {
        try {
            ...
            foreach ($_POST['item'] as $pergunta_id) {
                $pergunta = (new Tarefa())->buscar($pergunta_id);
                $pergunta->setOrdem($ordem);
                $pergunta->atualizar();
                $ordem++;
            }
            self::setMessage(TipoMensagem::SUCCESS, "Ordem de tarefas alterada.", $pergunta->getSetorFase()->getId(), true);
        }  catch (DBALException $ex) {
            self::setMessage(TipoMensagem::ERROR, "Erro ao ordenar tarefas.", null, true);
            parent::registerLogError($ex);
        }  catch (AppException $ex) {
            self::setMessage(TipoMensagem::ERROR, $ex->getMessage(), null, true);
        } catch (Exception $ex) {
            self::setMessage(TipoMensagem::ERROR, "Erro ao ordenar tarefas. Erro: {$ex->getMessage()}.", null, true);
            parent::registerLogError($ex);
        }
    }
