...
use const PDF_PARSER_ERROR_HANDLER_EXCEPTION;
use const PDF_PARSER_ERROR_HANDLER_PHP_DIE;
use const PDF_TYPE_ARRAY;
use const PDF_TYPE_BOOLEAN;
use const PDF_TYPE_DICTIONARY;
use const PDF_TYPE_HEX;
use const PDF_TYPE_NULL;
use const PDF_TYPE_NUMERIC;
use const PDF_TYPE_OBJECT;
use const PDF_TYPE_OBJREF;
use const PDF_TYPE_REAL;
...
// -----------------------------------------------------------------------------
    ...
    protected function getRawObject($offset = 0, $data = null)
    {
        ...
        if(!isset($data{$offset})){
            throw new Exception("[ERRO LOOP INFINITO] Uninitialized string offset: in /var/www/html/lib/pdf-merger/tcpdf/tcpdi_parser.php ");
        }
